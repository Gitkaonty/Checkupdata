...
exports.addSection = async (req, res) => {
    try {
        const { intitule, compte, id_dossier, id_compte, id_axe, section, par_defaut, fermer } = req.body;
...
exports.addOrUpdateSections = async (req, res) => {
    try {
        const { id, compte, compteId, fileId, axeId, intitule, section, fermer, par_defaut } = req.body;
        ...
        if (!compteId || !fileId || !intitule || !section) {
            return res.status(400).json({
                state: false,
                message: "Données manquantes ou invalides"
            });
...
exports.getSections = async (req, res) => {
    try {
        ...
        const data = await caSections.findAll({
            ...
            attributes: ['id', 'section', 'intitule', 'id_axe', 'compte' ],
            order: [
                ['id_axe', 'ASC'],
                ['id', 'ASC']
            ]
...
exports.importSections = async (req, res) => {
    try {
        ...
        for (let i = 0; i < sectionsData.length; i++) {
            ...
            if (!item.section || !item.intitule) {
                anomalies.push(`Ligne ${i + 1}: Données manquantes (section ou intitulé)`);
                continue;
            }
        ...
        const rows = await analytiques.findAll({
            ...
            include: [
                ...
                },
                {
                    model: caSections,
                    as: 'section',
                    attributes: ['id', 'intitule', 'section', 'compte'],
                }
            ],
